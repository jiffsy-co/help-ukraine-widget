...
export function scriptDatasetParams(): WidgetConfig {
    ...
    const widgetLayout = parseEnum(script?.getAttribute(scriptLayoutAttribute), slideLayout, defaultLayout);
...
export function scriptSrcParams(
	...
	try {
		...
		params.widgetPosition = parseEnum(url.searchParams.get('position'), widgetPositions, defaults.widgetPosition);
		params.widgetLayout = parseEnum(url.searchParams.get('layout'), slideLayout, defaults.widgetLayout);
	}
	catch (error) {
		console.error(error)
	}
