...
function sliderElement(widget: HTMLElement) {
  ...
  if (!(window as any).__HELPUKRAINEWIDGET_DISABLE_PERSISTENCE && persistedState) {
    widget.dataset.huwwSlide = persistedState
  }
  triggerList.forEach((triggerElem: HTMLElement) => {
    triggerElem.addEventListener('click', (e) => {
      e.preventDefault()
      const sliderState = triggerElem.dataset.huwwTrigger!
      widget.dataset.huwwSlide = sliderState
      sliderStorage.set(sliderState)
      try {
        ;(window as any).plausible &&
          (window as any).plausible('ChangeState', { props: { state: sliderState } })
      } catch (error) {}
    })
  })
  actionList.forEach((actionElem: HTMLElement) => {
    actionElem.addEventListener('click', (e) => {
      ...
      try {
        ;(window as any).plausible &&
          (window as any).plausible('Outbound Link: Click', {
            props: { url: actionElem.getAttribute('href') },
          })
      } catch (error) {}
    })
  })
